'''Trying with push intensity of 100. Earlier 230 had shown inconsistent results, while 0 had shown good results
checking for an arbitrary middle intensity 100. Mean-20 is retained from earlier'''

''' Lesser pixels pushed to zero doesnt help. Less pixel intensity like 0 seems reliable'''

'''Results dont show segmentation of tumor'''
...
''' Mean + Standard Deviation _/
    ...
    Push pixels below mean-20
    Push Intensity 100
    Image Modality and Patient T1CE BRATS 2021 and 00002
    Number of Images Used 20
    Results not availiable: https://drive.google.com/uc?export=download&id=1AhJptdUPgilHIUW8NpprQa8ll5vOXn4c'''
...
def some_val(im, pixel_value):
    ...
    for i in range(m):
        for j in range(n):
            ...
            a larger value'''
            if np.mean(mean_ij[i][j])<(pixel_value-20):
...
image_files = [0]*20
for i,num in enumerate([98,96,94,92,90,88,86,84,82,80,78,76,74,72,70,68,102,100,106,104]):
    image_files[i] = folder_dir+'/00002_'+str(num)+'.png'
